...
#loop for iterating through the graphs
GN=1
...
while(nos_index<=length(nos)){
	...
	load(file=paste("./Code, Datasets and Results/PBSI_S/S_",graph_names[GN],".RData",sep=''))
	#S=S_FB
	#plot(graph)
	out_min_sum=0
	s=list()
	...
	Y[c(infected_nodes_object[[k]])]=c(rep(1,length(infected_nodes_object[[k]])))
	df=NULL
	...
	while(it<=length(Z)) {
		...
		{
			n_s=n_s+1
			indices=append(indices,as.character(V(graph)[it]$name))
		}
	}
		it=it+1
	...
	print(paste("number of estimated sources: ",n_s[1]))
...
write.csv(df,paste("./Code, Datasets and Results/Results/PBSI/",object,".csv",sep=''),sep='\t',row.names=F)
