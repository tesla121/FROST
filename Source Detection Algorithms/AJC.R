...
for(node in as.character(V(sg)$name) ){
	if(length(which(Ai[node,]==1))>=floor(mean(degree(sg))) ){
		K=append(K,node)
	}
}
K=unlist(K)
...
write.csv(df,paste("./Code, Datasets and Results/Results/AJC/",object,".csv",sep=''),sep='\t',row.names=F)
